(function () {
  ...
  function workspaceActionsMarkup() {
    ...
    const disabled = Boolean(actionInFlight || state.tenantLoading);
    return `
      <button class="board-action" data-workspace-action="sync" ${(enabledActions.includes("sync") && !disabled) ? "" : "disabled"}>${actionInFlight === "sync" ? "Syncing..." : "Sync"}</button>
      <button class="board-action" data-workspace-action="preview" ${(enabledActions.includes("preview") && !disabled) ? "" : "disabled"}>${actionInFlight === "preview" ? "Starting..." : "Preview"}</button>
      <button class="board-action" data-workspace-action="discard" ${(enabledActions.includes("discard") && !disabled && workspace.dirty) ? "" : "disabled"}>${actionInFlight === "discard" ? "Discarding..." : "Discard Changes"}</button>
      <button class="board-action" data-workspace-action="deploy" ${(enabledActions.includes("deploy") && !disabled) ? "" : "disabled"}>${actionInFlight === "deploy" ? "Deploying..." : "Deploy"}</button>
      <button class="board-action" id="refreshWorkspaceButton" type="button" ${disabled ? "disabled" : ""}>Refresh</button>
    `;
  }
        ...
        <main class="panel shared-board">
          <header class="shared-board-header">
            ...
            <div class="shared-board-controls">
              ...
              <div class="shared-utility-actions">
                <button class="secondary" id="toggleComposerButton" type="button" ${state.tenantLoading ? "disabled" : ""}>${state.composerOpen ? "Hide Request" : "New Request"}</button>
                ${userCanViewAdminPanel() ? `<button class="secondary" id="toggleAdminButton" type="button" ${state.tenantLoading ? "disabled" : ""}>${state.adminOpen ? "Hide Admin" : "Admin"}</button>` : ""}
          ...
          <section class="shared-drawer ${state.composerOpen ? "" : "hidden"}" id="sharedComposerPanel">
            <form id="sharedRequestForm" class="request-form shared-inline-form" ${passwordManagerIgnoreAttrs()}>
              ...
              <div class="form-actions">
                <button type="submit" ${state.tenantLoading ? "disabled" : ""}>Queue Task</button>
  ...
  async function submitRequest(event) {
    ...
    const submitButton = form.querySelector('button[type="submit"]');
    if (!tenant || state.requestSubmitInFlight || state.tenantLoading) return;
  ...
  async function handleRequestAction(event) {
    ...
    if (!tenant || state.tenantLoading || !requestId || !["cancel", "archive", "retry"].includes(action)) {
      return;
    }
    const key = requestActionKey(requestId, action);
  ...
  async function submitReply(event) {
    ...
    if (!tenant || state.tenantLoading || !requestId || (!text && !draft.files.length)) {
      return;
    }
    try {
      const payload = new FormData();
  ...
  async function runWorkspaceAction(action) {
    const tenant = activeTenant();
    if (!tenant || state.tenantLoading) return;
